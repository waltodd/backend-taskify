...
export const createTask = async (req, res) => {
  ...
  try {
    const task = new Task({ title, priority, userId: req.user.userId }); // Atribui o ID do usuário à tarefa
    await task.save();
...
export const getTasks = async (req, res) => {
  try {
    const tasks = await Task.find({ userId: req.user.userId }); // Busca todas as tarefas do usuário
...
export const getTaskById = async (req, res) => {
  const { id } = req.params;
  try {
    const task = await Task.findOne({ _id: id, userId: req.user.userId }); // Busca a tarefa pelo ID e pelo ID do usuário
...
export const deleteTask = async (req, res) => {
  const { id } = req.params;
  try {
    const task = await Task.findOneAndDelete({ _id: id, userId: req.user.userId }); // Remove a tarefa pelo ID e pelo ID do usuário
    if (!task) return res.status(404).json({ message: "Tarefa não encontrada" }); // Retorna 404 se não encontrar
...
export const filterTasksByPriority = async (req, res) => {
  ...
  try {
    const tasks = await Task.find({ userId: req.user.userId, priority }); // Filtra tarefas pela prioridade e ID do usuário
...
export const filterTasksByStatus = async (req, res) => {
  ...
  try {
    const tasks = await Task.find({ userId: req.user.userId, completed }); // Filtra tarefas pelo status e ID do usuário
    res.status(200).json(tasks); // Retorna as tarefas filtradas
  } catch (err) {
    res.status(500).json({ error: err.message }); // Lida com erros
  }
